...
//            self.rootVC?.view.transform = CGAffineTransform(translationX: self.menuWidth, y: 0)
            ...
            // 加入蒙版
            GalaxyCover.show(frame: (self.rootVC?.view.frame)!, type: .clear)
            self.addGesture()
        }
    }
...
extension DrawerViewController {
    ...
    fileprivate func addGesture() {
        ...
        GalaxyCover.shared.addGestureRecognizer(panGesture)
        GalaxyCover.shared.addGestureRecognizer(tapGesture)
    }
